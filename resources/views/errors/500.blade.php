...
<head>
    ...
    <title>500 - Server Error | {{ config('app.name', 'DentalCare') }}</title>
...
<body class="h-full font-sans antialiased text-slate-100 bg-slate-950 flex flex-col items-center justify-center p-6 relative overflow-hidden">
    <div class="max-w-md w-full bg-slate-900/90 border border-slate-800 backdrop-blur-2xl rounded-3xl p-8 sm:p-10 shadow-2xl relative z-10 text-center space-y-6">
        <div class="mx-auto w-16 h-16 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 font-extrabold text-2xl">
            500
        </div>

        <div class="space-y-2">
            <h1 class="text-2xl sm:text-3xl font-bold tracking-tight text-white">Server Error</h1>
            <p class="text-sm text-slate-400 leading-relaxed">An unexpected internal server error occurred. Please try again shortly.</p>
        </div>
