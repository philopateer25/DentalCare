...
export default function WelcomeNavbar({ auth, appName = 'DentalCare', onMenuClick }) {
    const { t } = useLocale();
        ...
        <header className="relative z-20 border-b border-neutral-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-950/70 backdrop-blur-xl sticky top-0 transition-colors">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">
                ...
                <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-neutral-600 dark:text-slate-400">
                    <a href="#features" className="hover:text-black dark:hover:text-teal-300 transition-colors">{t('Features')}</a>
                    <a href="#booking" className="hover:text-black dark:hover:text-teal-300 transition-colors">{t('Book Online')}</a>
                    <Link href="/Pricing" className="hover:text-black dark:hover:text-teal-300 transition-colors">{t('Pricing')}</Link>
                </nav>
                ...
                <div className="flex items-center gap-2.5 sm:gap-3">
                    ...
                    <a href="/auth/Register" className="hidden sm:inline-flex">
