...
import { CONDITION_COLORS, CONDITION_LABELS, ToothRecord } from './types';
...
// Check if node is a structural/bounding box artifact
...
const GLTFDentalArch: React.FC<{
  ...
  // RULE ENFORCEMENT: Clone scene to preserve world transforms and prepare unique materials
  ...
  // RULE ENFORCEMENT: Apply main condition colors and styles. NO surface painting logic.
  useEffect(() => {
  ...
  return (
    <group>
      {/* RULE ENFORCEMENT: Using onDoubleClick instead of onPointerDblClick */}
      <primitive
        object={scene}
        onPointerDown={(e: any) => {
          e.stopPropagation();
...
// RULE ENFORCEMENT: Error Boundary / Fallback Wrapper Component
...
export const Dental3DViewer: React.FC<Dental3DViewerProps> = ({
  ...
  return (
    <div className={`relative w-full h-full min-h-[450px] overflow-hidden ${cleanMode ? 'bg-transparent' : 'bg-gradient-to-b from-slate-950 via-slate-900 to-zinc-950 rounded-2xl shadow-2xl border border-slate-800/80'}`}>
